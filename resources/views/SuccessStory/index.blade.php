...
@section('page_header')
        ...
        <h1 class="page-title">
            <i class="voyager-people"></i> Success Stories
        </h1>
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-footer">
                <form action="{{route('story.bulkDelete')}}" id="bulk_delete_form" method="POST">
                    ...
                    <div class="panel-body">
                            ...
                            <th class="actions text-right">Actions</th>
                                ...
                                <tbody>
                                    ...
                                    @foreach($allSuccessStories as $story)
                                 <tr><td>
                                  <input type="checkbox" name="row_id" id="checkbox_{{$story->id}}" value="{{$story->id}}">
                               </td>
                              
                               
                       <td>
                           {{ $story->submitUser->name}}
                       </td>
                       <td>
                           {{ $story->partner->name}}
                       </td>
                       ...
                       <td class="no-sort no-click" id="bread-actions">
                           <a href="javascript:;" title="Delete" class="btn btn-sm btn-danger pull-right delete"data-id="{{$story->id}}" id="delete-{{$story->id}}">
            <i class="voyager-trash"></i> <span class="hidden-xs hidden-sm">Delete</span>
            <a href="{{route('story.edit',['id'=>$story->id])}}" title="Edit" class="btn btn-sm btn-primary pull-right edit">
            <i class="voyager-edit"></i> <span class="hidden-xs hidden-sm">Edit</span>
        </a>
            <a href="{{route('story.detail',['id'=>$story->id])}}" title="View" class="btn btn-sm btn-warning pull-right view">
...
@section('javascript')
       ...
       <script>
        ...
        $('td').on('click', '.delete', function (e) {
            $('#delete_form')[0].action = {!! json_encode(url("/Success-Stories/Delete/__id")) !!}.replace('__id', $(this).data('id'));
